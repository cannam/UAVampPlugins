...
MF0::MF0(const pinfo &info,int maxpolyphony, float lowestnote, float highestnote)
{
  printlog=false;
  this->info=info;
...
double MF0::midi2freq(int midinote) {
...
double MF0::centeredfreq(double f) {
...
bool MF0::isclosetoband(double freq)
{
...
double MF0::selectpeakcloseto(mapa2 &spectrumpeaks,double centerfreq, int nh, double &maxval) 
{
...
mapa2 MF0::GetF0Candidates(mapa2 spectrumpeaks) 
{
    mapa2 candidates;
    mapa3 prob;
    candidates.clear();
...
double MF0::equilibrateloudness(Combination &comb)
{
  double mean=0;
  double diff=0;
...
void MF0::ComputeCombinationsSalience (mapa2 &spectrumpeaks, mapa2 &f0candidates, vcombinations &comb) 
{
...
void MF0::AddBestCombination(Combination &bestcombination, vectorchords &bestmelodynotes, int tini, int tend)
{
...
void MF0::AddBestCombination(const vint &bestcombination, vectorchords &bestmelodynotes, int tini, int tend)
{
    // Convert combination into midi notes for writemidi function
    notesvector notesok;
    notesok.reserve(bestcombination.size());
...
mapa4 MF0::CleanFrameCombinations(vcombinations &vc) // by reference for efficiency
{
      mapa4 cleaned;
...
vsimplecombinations MF0::CleanFrameCombinationsLoudness(vcombinations &vc) // by reference for efficiency
{
...
double MF0::selectHighestSalience(mapa4 &m, itermapa4 &bestcomb) // by reference for eficiency
{
...
vint MF0::MergeFramesCombinations(const deque<mapa4> &qc)
{
    mapa4 m;
    double bestsalience;
    itermapa4 bestcomb;
...
vsimplecombinations MF0::MergeFramesCombinationsLoudness(const deque<vsimplecombinations> &qc, double &maxenergy) // qc by reference for efficiency
{
...
void MF0::applyhypothesisonsets(peaksattvector &peaks, vectorchords &bestmelodynotes) 
{
	for (iterpeaksattvector curr=peaks.begin(); curr!=peaks.end(); curr++)
	{
		...
		vcombinations comb= GetAllCombinations(f0candidates,maxpolyphony);
...
void MF0::computeTempoChanges(vector<double> beatsvector, double tempochanges[], int size)
{
...
void MF0::quantizebeats(vector<double> beatsvector, bool quantized[], int sizequantized)
{
...
void MF0::quantizeonsets(vector<double> onsetsvector, vector<bool> &quantized, float time_resolution)
{
  for (unsigned int i=0; i<quantized.size(); i++)
    quantized[i]=false;

  for (unsigned int i=0; i<onsetsvector.size(); i++)
  {
    unsigned int index=(int)round(onsetsvector[i]/time_resolution);
    if (index<quantized.size())
      quantized[index]=true;
    else cout << "Error" << endl;
  } 
  
  if (printlog) {
    cout << "SIZEQ=" << quantized.size() << endl;
    for (unsigned int i=0; i<quantized.size(); i++)
      if (quantized[i]) cout << i*time_resolution << endl;
  }
}
...
void MF0::cleannotes(vectorchords& melodychords)
{
...
void MF0::applyhypothesisbeat(peaksattvector &peaks, vectorchords &bestmelodynotes, vector<bool> quantized)
{
	...
	for (iterpeaksattvector curr=peaks.begin(); curr!=peaks.end(); curr++)
	{
		...
		if (!comb.empty()) { 
			ComputeCombinationsSalience(curr->specpeaks, f0candidates, comb);
                ...
                if (nframes+halfwindow>=(int)quantized.size() || quantized[nframes+halfwindow]==true)
                {
...
void MF0::printloudnessmatrix(vsimplecombinations &vbestcomb)
{
...
double MF0::noteenergydiff(const SimpleCombination& sc1, const SimpleCombination &sc2)
{
  double energydiff=0;
  vector<int> all;
  int notfound=0;
...
double MF0::getEdgeWeight(const SimpleCombination& sp1, const SimpleCombination& sp2)
{
  double weight;
...
void MF0::applyhypothesisfixsp(peaksattvector &peaks, vectorchords &bestmelodynotes, bool plotintensity, bool printgraph, const char *graphfilename)
{
	...
	if (plotintensity) printloudnessmatrix(vbestcomb);
        ...
        vector<int> shortestpath=MF0Graph::buildgraph(&nodenames[0], numnodes, &edges[0], &weights[0], (int)edges.size(), printgraph, graphfilename);
...
void MF0::createNode(SimpleCombination &sc, unsigned long &numnodes, int nframes, vector<string> &nodenames, vector<map <int, SimpleCombination> > &allmergedcombinations)
{
        sc.setid(numnodes);
...
SimpleCombination MF0::predictCombination(const SimpleCombination& sc1, const SimpleCombination& sc2)
{
  vector<int> pitches;
...
void MF0::applyhypothesisfixsp3(peaksattvector &peaks, vectorchords &bestmelodynotes, bool plotintensity, bool printgraph, const char *graphfilename)
{
        deque<vsimplecombinations> combinations;
...
void MF0::applyhypothesisfbf(peaksattvector &peaks, vectorchords &bestmelodynotes, bool eval)
{
        deque<mapa4> allcombinations;
        int nframes=0;
...
void MF0::getpeaksvector (int t_ini, int t_end, peaksvector &allpeaks, peaksattvector &peaks, peaksattvector &previouspeaks) // allpeaks by reference for efficency
{
    int t;
...
void MF0::printbestnotes(const vectorchords &bestmelodynotes) {
...
void MF0::printMirexFBF(const vectorchords &melodychords) 
{
...
void MF0::printMirexNTrack(const vectorchords &melodychords) 
{
        map<int,double> activenotes;	// Detected pitches
        map<int,double> tmpactivenotes;
...
void MF0::printeval(const vectorchords &bestmelodynotes)
{
...
void MF0::processonsets(int t_end, double *onsetsvector, peaksvector &allpeaks, peaksattvector &peaks, peaksattvector &previouspeaks)  // allpeaks by reference for efficency
{	
	int lasttime=0;
...
vector<double> MF0::LoadBeatsFile(string filename)
{
  ifstream f;
  double data;
  vector<double> beats;
...
void MF0::writeonsetsvector(vector<double> in, double out[], int n_frames, double timeres)
{
